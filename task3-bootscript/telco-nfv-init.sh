...
# telco-nfv-init.sh
# 
# Telco/NFV Boot Initialization Script
# Chạy lúc khởi động để cấu hình hệ thống cho NFV workloads
#
# Features:
#   1. HugePages allocation & verification
#   2. Network IP configuration (DHCP/Static)
#   3. Kernel module loading (VFIO, vhost_net)
#   4. CPU isolation verification
#   5. NUMA topology check
...
#-----------------------------------------------------------------------------
# CONFIGURATION — Có thể override bằng /etc/telco-nfv/config
#-----------------------------------------------------------------------------
# HugePages
HUGEPAGES_2M=${HUGEPAGES_2M:-1024}          # Số lượng 2MB hugepages (= 2GB RAM)
HUGEPAGES_1G=${HUGEPAGES_1G:-0}             # Số lượng 1GB hugepages (= 0 mặc định)
HUGEPAGE_MOUNT="/mnt/huge"                   # Mount point cho hugetlbfs

# Network
NET_MODE=${NET_MODE:-"dhcp"}                 # "dhcp" hoặc "static"
NET_IFACE=${NET_IFACE:-"eth0"}               # Interface chính
NET_IP=${NET_IP:-"192.168.1.100/24"}         # Static IP (nếu mode=static)
NET_GATEWAY=${NET_GATEWAY:-"192.168.1.1"}    # Default gateway
NET_DNS=${NET_DNS:-"8.8.8.8"}               # DNS server

# Kernel Modules
TELCO_MODULES="vfio vfio-pci vhost_net tun tap bonding 8021q bridge"

# CPU Isolation (đọc từ kernel cmdline)
# isolcpus=2-3 → CPUs 2,3 dành riêng cho DPDK

# Logging
LOG_FILE="/var/log/telco-nfv-init.log"
...
configure_hugepages() {
    ...
    if [[ -f "${hp_path}" ]]; then
        # Đọc giá trị hiện tại (có thể đã set qua kernel cmdline)
        local current
        current=$(cat "${hp_path}")
...
configure_network() {
    ...
    if ! ip link show "${NET_IFACE}" &>/dev/null; then
        ...
        # Tự động chọn interface đầu tiên (không phải lo)
        local auto_iface
    ...
    case "${NET_MODE}" in
        dhcp)
            ...
            # Dùng wicked (SUSE) hoặc dhclient fallback
...
WICKED
            ...
            # Fallback: dhclient
            if ! ip addr show "${NET_IFACE}" | grep -q "inet "; then
...
load_kernel_modules() {
    ...
    lsmod | grep -E "vfio|vhost|tun|tap|bond|8021q|bridge" | while read -r line; do
        log_info "  ${line}"
    done
}
...
verify_cpu_isolation() {
    ...
    # Đọc isolcpus từ kernel cmdline
    local cmdline
    cmdline=$(cat /proc/cmdline 2>/dev/null || echo "")
    
    local isolated_cpus=""
    if [[ "${cmdline}" =~ isolcpus=([^ ]+) ]]; then
        isolated_cpus="${BASH_REMATCH[1]}"
        log_ok "Isolated CPUs (from cmdline): ${isolated_cpus}"
    else
...
# Tăng buffer để xử lý burst traffic (line-rate 10G/25G)
...
# --- Memory ---
# Disable swap (Telco = low-latency, swap = bad)
vm.swappiness = 0
...
# Panic on OOM instead of random kill (predictable failure)
vm.panic_on_oom = 0
...
# Disable transparent hugepages (conflicts with DPDK explicit hugepages)
# Note: also set via kernel cmdline: transparent_hugepage=never
SYSCTL

    # Apply
    sysctl --system 2>/dev/null | grep -c "applied" | while read -r count; do
        log_ok "Sysctl: ${count} settings applied"
    done
    
    # Disable Transparent HugePages (THP) — conflicts with DPDK
